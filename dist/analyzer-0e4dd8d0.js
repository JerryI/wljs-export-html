...
class AnalyzerNode {
    ...
    constructor(log) {
        ...
        this.log.forEach((el) => {
            if (el.uid) tokens.series.push([0, eventNameToString(el)]);
            else tokens.series.push([1, String(el.name)]);
          });
    ...
    groups(keys = ['count', 'probabilities', 'connections']) {
        return this.groups.map((group) => {
            ...
            Object.keys(group.probabilities).forEach((k) => {
                ...
                Object.keys(group.count[k]).forEach((kk) => {
                    tranformed.probabilities[this.tokens.fromLetters[k]][this.tokens.fromDigits[kk]] = group.count[k][kk];
                });
            });
